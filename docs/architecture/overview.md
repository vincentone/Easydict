...
Easydict 是一款 macOS 词典和翻译应用，支持直接查词、文本翻译、划词翻译和
OCR 截图翻译，查询来源为有道翻译服务。Apple Vision OCR 与系统语言检测作为
离线基础能力保留，不作为独立来源展示。
...
- 查询服务在 `Easydict/Swift/Service` 下实现请求和响应解析；服务注册表
  `QueryServiceFactory` 当前只注册有道服务。
